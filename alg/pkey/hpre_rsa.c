...
static EVP_PKEY_METHOD *setup_pkey_meth(void)
{
    ...
    unsigned char *to, RSA *rsa, int padding)
{
    ...
    eng_ctx->opdata.in = eng_ctx->rsa_setup.ops.alloc(eng_ctx->qlist->kae_queue_mem_pool,
        eng_ctx->qlist->kae_queue_mem_pool->block_size);
    eng_ctx->opdata.out = eng_ctx->rsa_setup.ops.alloc(eng_ctx->qlist->kae_queue_mem_pool,
        eng_ctx->qlist->kae_queue_mem_pool->block_size);
