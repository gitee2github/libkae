...
int wd_hpre_init_qnode_pool()
{
    ...
    g_hpre_rsa_qnode_pool = kae_init_queue_pool(WD_RSA);
...
static int hpre_init_eng_ctx(hpre_engine_ctx_t *eng_ctx, int bits)
{
    ...
    if (eng_ctx->ctx == NULL) {
        ...
        eng_ctx->rsa_setup.ops.alloc = kae_wd_alloc_blk;
        eng_ctx->rsa_setup.ops.free = kae_wd_free_blk;
        eng_ctx->rsa_setup.ops.dma_map = kae_dma_map;
        eng_ctx->rsa_setup.ops.dma_unmap = kae_dma_unmap;
        eng_ctx->rsa_setup.ops.usr = pool;
...
void hpre_free_eng_ctx(hpre_engine_ctx_t *eng_ctx)
{
    ...
    if (eng_ctx->opdata.op_type != WCRYPTO_RSA_GENKEY) {
        if (eng_ctx->opdata.in) {
            eng_ctx->rsa_setup.ops.free(eng_ctx->qlist->kae_queue_mem_pool, eng_ctx->opdata.in);
        }
        if (eng_ctx->opdata.out) {
            if (eng_ctx->qlist != NULL) {
                eng_ctx->rsa_setup.ops.free(eng_ctx->qlist->kae_queue_mem_pool, eng_ctx->opdata.out);
            }
        }
    } else {
        if (eng_ctx->opdata.in) {
    ...
    RSA *rsa, BIGNUM *e_value, BIGNUM *p, BIGNUM *q)
{
    ...
    BN_bin2bn((unsigned char *)wd_qinv.data, key_size / 2, iqmp);   // qinv size is key_size / 2
    BN_bin2bn((unsigned char *)wd_dq.data, key_size / 2, dmq1);     // dq size is key_size / 2
    BN_bin2bn((unsigned char *)wd_dp.data, key_size / 2, dmp1);     // dp size is key_size / 2
