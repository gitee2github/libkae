...
void wd_ciphers_free_engine_ctx(void* engine_ctx)
{
    ...
    if (e_cipher_ctx->op_data.in && e_cipher_ctx->setup.ops.usr) {
        e_cipher_ctx->setup.ops.free(e_cipher_ctx->setup.ops.usr, (void *)e_cipher_ctx->op_data.in); 
        e_cipher_ctx->op_data.in = NULL;
    }
    
    if (e_cipher_ctx->op_data.out && e_cipher_ctx->setup.ops.usr) {
        e_cipher_ctx->setup.ops.free(e_cipher_ctx->setup.ops.usr, (void *)e_cipher_ctx->op_data.out); 
        e_cipher_ctx->op_data.out = NULL;
    }

    if (e_cipher_ctx->op_data.iv && e_cipher_ctx->setup.ops.usr) {
        e_cipher_ctx->setup.ops.free(e_cipher_ctx->setup.ops.usr, (void *)e_cipher_ctx->op_data.iv); 
...
static cipher_engine_ctx_t* wd_ciphers_new_engine_ctx(KAE_QUEUE_DATA_NODE_S* q_node, cipher_priv_ctx_t* priv_ctx)
{
    ...
    e_cipher_ctx->setup.ops.alloc = kae_wd_alloc_blk;
    e_cipher_ctx->setup.ops.free = kae_wd_free_blk;
    e_cipher_ctx->setup.ops.dma_map = kae_dma_map;
    e_cipher_ctx->setup.ops.dma_unmap = kae_dma_unmap;
    e_cipher_ctx->setup.ops.usr = q_node->kae_queue_mem_pool;

    e_cipher_ctx->op_data.in = e_cipher_ctx->setup.ops.alloc(e_cipher_ctx->setup.ops.usr, INPUT_CACHE_SIZE); 
    if (e_cipher_ctx->op_data.in == NULL) {
        US_ERR("alloc opdata in buf failed");
        goto err;
    }

    e_cipher_ctx->op_data.out = e_cipher_ctx->setup.ops.alloc(e_cipher_ctx->setup.ops.usr, OUTPUT_CACHE_SIZE); 
    if (e_cipher_ctx->op_data.out == NULL) {
        US_ERR("alloc opdata out buf failed");
        goto err;
    }

    e_cipher_ctx->op_data.iv = e_cipher_ctx->setup.ops.alloc(e_cipher_ctx->setup.ops.usr, priv_ctx->iv_len); 
...
int wd_ciphers_init_qnode_pool(void)
{
    ...
    g_sec_ciphers_qnode_pool = kae_init_queue_pool(WD_CIPHER);
