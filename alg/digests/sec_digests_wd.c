...
void wd_digests_free_engine_ctx(void* digest_ctx)
{
    ...
    if (e_digest_ctx->op_data.in && e_digest_ctx->setup.ops.usr) {
        e_digest_ctx->setup.ops.free(e_digest_ctx->setup.ops.usr, (void *)e_digest_ctx->op_data.in); 
        e_digest_ctx->op_data.in = NULL;
    }
    
    if (e_digest_ctx->op_data.out && e_digest_ctx->setup.ops.usr) {
        e_digest_ctx->setup.ops.free(e_digest_ctx->setup.ops.usr, (void *)e_digest_ctx->op_data.out); 
...
static digest_engine_ctx_t* wd_digests_new_engine_ctx(KAE_QUEUE_DATA_NODE_S* q_node, sec_digest_priv_t* md_ctx)
{
    ...
    e_digest_ctx->setup.ops.alloc = kae_wd_alloc_blk;
    e_digest_ctx->setup.ops.free = kae_wd_free_blk;
    e_digest_ctx->setup.ops.dma_map = kae_dma_map;
    e_digest_ctx->setup.ops.dma_unmap = kae_dma_unmap;
    e_digest_ctx->setup.ops.usr = q_node->kae_queue_mem_pool;

    e_digest_ctx->op_data.in = e_digest_ctx->setup.ops.alloc(e_digest_ctx->setup.ops.usr, DIGEST_BLOCK_SIZE); 
    if (e_digest_ctx->op_data.in == NULL) {
        US_ERR("alloc opdata in buf failed");
        goto err;
    }

    e_digest_ctx->op_data.out = e_digest_ctx->setup.ops.alloc(e_digest_ctx->setup.ops.usr, DIGEST_BLOCK_SIZE); 
...
void wd_digests_set_input_data(digest_engine_ctx_t *e_digest_ctx)
{
    ...
    e_digest_ctx->op_data.out_bytes = HASH_LEN;
...
inline void wd_digests_get_output_data(digest_engine_ctx_t *e_digest_ctx)
{
    ...
    if (e_digest_ctx->op_data.has_next == false) {
        kae_memcpy(md_ctx->out, (uint8_t*)e_digest_ctx->op_data.out, HASH_LEN);
    }
}
...
int wd_digests_init_qnode_pool(void)
{
    ...
    g_sec_digests_qnode_pool = kae_init_queue_pool(WD_DIGEST);
